      ...
      function openCustomer(id, options = {}) {
        ...
        const people = [...activePeople, ...stoppedPeople];
        openDrawer(
          `<div class="drawer-head"><div class="modal-title">客户单位详情</div><button class="icon-btn close" data-close title="关闭">×</button></div><div class="drawer-body"><div class="detail-hero customer-drawer-hero"><div class="avatar">企</div><div><div class="detail-name">${customer.name}</div><div class="detail-sub">${customerOrganizationPath(customer)} · 业务责任 ${customerBusinessResponsibilityLevel(customer)} / ${adminArea(customer)}</div></div><div class="spacer"></div>${healthTag(customerHealth(customer))}${customer.responsibilityAnomaly ? '<span class="tag red">责任配置异常</span>' : ""}</div><div class="company-overview customer-drawer-overview"><div class="overview-item"><label>集团公司</label><div>${customer.group}</div></div><div class="overview-item"><label>行业</label><div>${customer.industry}</div></div><div class="overview-item"><label>组织上级</label><div>${customerOrganizationParent(customer)?.name || customer.group}</div></div><div class="overview-item"><label>业务责任层级</label><div>${customerBusinessResponsibilityLevel(customer)}</div></div><div class="overview-item"><label>业务责任省/市/区县</label><div>${adminArea(customer)}</div></div><div class="overview-item"><label>区域中心</label><div>${customerRegionLabel(customer)}</div></div><div class="overview-item"><label>客户负责人</label><div>${customerOwnerName(customer)}</div></div><div class="overview-item"><label>有效关键人</label><div>${activePeople.length} 人</div></div><div class="overview-item"><label>写入来源 / 创建时间</label><div>${customer.source === "import" ? "批量导入" : "手工录入"} · ${customer.createdAt || "2026-01-01 09:00"}</div></div></div>${customer.responsibilityAnomaly ? `<div class="role-note danger-note">客户负责人配置异常，请联系管理员。</div>` : ""}<div class="customer-drawer-section-head"><div class="section-title">关键人</div><span class="tag blue">${people.length} 人</span></div><div class="customer-drawer-contacts">${people.map((person) => `<div class="customer-drawer-contact"><div class="avatar">${person.name[0]}</div><div class="list-main"><div class="list-title">${person.name} · ${person.title}${person.decision ? ' <span class="tag blue">关键决策人</span>' : ""}${person.archived ? ' <span class="tag red">已停用</span>' : contactHasOverdue(person) ? ' <span class="tag red">当前逾期</span>' : ""}</div><div class="list-sub">${person.department} · ${person.positionName} · ${person.level}</div></div>${options.limitContacts ? '<span class="list-sub">下钻止于客户单位详情，查看关键人请返回客户经营</span>' : `<button class="link" data-person="${person.id}">查看详情</button>`}</div>`).join("") || '<div class="role-note">暂无关键人。</div>'}</div></div><div class="drawer-foot">${canMaintainContactForCompany(customer) ? `<button class="btn btn-primary" data-action="add-contact" data-id="${customer.id}">新增关键人</button>` : ""}</div>`,
        );
      }
