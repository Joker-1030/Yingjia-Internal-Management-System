# FLOW-17：M12 销售与商机管理

- 状态：Current
- Owner：M12
- Decision：DEC-068、DEC-070、DEC-072、DEC-074、DEC-075、DEC-081、DEC-194、DEC-204

## 销售指标

```mermaid
flowchart TD
    A[设置自然年度逐月商机数量目标] --> B{上下级守恒?}
    B -- 否 --> C[指出月份和差额并禁止发布]
    B -- 是 --> D{当前角色和调整层级有权?}
    D -- 否 --> H[阻止提交,原版本继续生效]
    D -- 是 --> F[用户确认]
    F --> E[生成并生效新指标版本]
```

首次区域分配由市场副总完成，首次 PM 分配由区域总监完成；总裁和系统管理员可覆盖管理。已发布版本调整继续按原角色层级、当前月/未来月和上下级守恒规则，由 M12 校验并在确认后直接形成新生效版本，不进入 M04。

## 商机主流程

```mermaid
stateDiagram-v2
    [*] --> 商机录入: 创建成功
    商机录入 --> 需求确认
    需求确认 --> 方案支撑
    方案支撑 --> 比选
    比选 --> 中选
    商机录入 --> 落选
    需求确认 --> 落选
    方案支撑 --> 落选
    比选 --> 落选
    中选 --> 落选
```

- “线索”只描述商机录入、需求确认等早期商机，不是独立对象或状态。
- 仅当前负责人可正常推进；区域总监先合法改派，再由新负责人继续。
- 进入中选填写中选日期和预计签约金额；中选仍允许跟进。
- 进入落选填写落选原因、竞争对手和复盘说明；落选后停止推进和新增跟进。
- 当前 M12 不进入已签约；合同与财务能力须由未来独立 Current 产品链接管。

## 跟进与方案支撑

```mermaid
flowchart TD
    A[创建商机并形成首次跟进日期] --> B[负责人新增结构化跟进]
    B --> C[生成新的下次跟进日期]
    C --> D{到期仍无有效跟进?}
    D -- 否 --> B
    D -- 是 --> E[当日上午提醒负责人]
    E --> F[次日起每日提醒]
    F --> G{逾期满 3 个自然日?}
    G -- 是 --> H[同步区域总监]
    G -- 否 --> F
    H --> B
```

方案支撑由商机负责人向一人或多人发起；每名支撑人员独立经历待响应、已响应、支撑中、已交付、已关闭。首次回应超时只产生 P01 消息，不改变商机阶段或自动改派。

## 失败保护

- 必填缺失、范围变化、关键人失效或非法阶段请求不改变当前事实。
- 并发阶段请求只有基于最新版本的合法请求生效。
- 指标校验、权限或并发失败不覆盖最近生效版本。
- 消息失败不回滚商机、跟进或支撑事实。
- 越权访问统一返回 403 且不泄露对象是否存在。
