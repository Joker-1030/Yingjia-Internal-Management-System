      function openTaskDetail(id) {
        ...
        const resumeTimeline = (t.resumeHistory || [])
          .map(
            (item) =>
              `<div class="timeline-item"><div class="timeline-title">${item.resumedAt} · 常规维系已恢复</div><div class="timeline-content">截止：${item.previousDue} → ${item.currentDue} 23:59:59；恢复时职级：${item.level}；周期：${item.cycleDays} 个自然日</div></div>`,
          )
          .join("");
        const pauseTimeline =
          t.status === "paused"
            ? t.inheritedPause
              ? `<div class="timeline-item"><div class="timeline-title">${escapeHtml(t.createdAt || DEMO_TODAY)} · 继承暂停已生效</div><div class="timeline-content">来源执行项于 ${escapeHtml(t.pausedAt || "—")} 确认暂停；计划 ${escapeHtml(t.resumeDate || "—")} 恢复</div></div>`
              : `<div class="timeline-item"><div class="timeline-title">${t.pausedAt || "2026-08-10 10:00"} · 暂停已确认</div><div class="timeline-content">计划 ${t.resumeDate} 恢复</div></div>`
            : "";
        const changeActionLabel = taskCanPause(t)
          ? "延期/取消/暂停"
          : "延期/取消";
        openDrawer(
          `<div class="drawer-head"><div class="modal-title">任务详情</div><button class="icon-btn close" data-close>×</button></div><div class="drawer-body"><div class="detail-hero"><div class="avatar">任</div><div><div class="detail-name">${t.title}</div><div class="detail-sub">${taskStatusName(t.status, t)}</div></div></div><div class="detail-grid"><div class="detail-item"><label>任务编号</label><div>${t.parentTaskCode}</div></div><div class="detail-item"><label>任务执行记录编号</label><div>${t.executionCode}</div></div>${handoverRelationFields}<div class="detail-item"><label>执行目标</label><div>${t.person}</div></div><div class="detail-item"><label>执行人</label><div>${t.pm} · ${t.executorRole || (t.company?.includes("有限公司") ? "区域总监" : "PM")}</div></div><div class="detail-item"><label>客户范围</label><div>${t.company}</div></div><div class="detail-item"><label>任务类型</label><div>${t.type}</div></div><div class="detail-item"><label>生成时间</label><div>${t.createdAt || "2026-08-01 09:00"}</div></div><div class="detail-item"><label>更新时间</label><div>${t.updatedAt || t.createdAt || "-"}</div></div><div class="detail-item"><label>原截止时间</label><div>${t.originalDue || t.due} 23:59:59</div></div><div class="detail-item"><label>当前截止时间</label><div>${t.due} 23:59:59</div></div><div class="detail-item full"><label>执行要求</label><div>${taskRequirementText(t)}</div></div><div class="detail-item full"><label>逾期补完成</label><div>${completionPolicy.allowed ? completionPolicy.cutoff ? `允许补完成至 ${completionPolicy.cutoff} 23:59:59` : "允许（无截止日期）" : "不允许"}</div></div>${t.resumeDate ? `<div class="detail-item"><label>暂停至</label><div>${t.resumeDate}</div></div>` : ""}${t.closeReason ? `<div class="detail-item"><label>${autoClosed ? "关闭原因" : "取消原因"}</label><div>${escapeHtml(t.closeReason)}</div></div>` : ""}${employeeStopClosure}</div>${completionAudit}<div class="section-title">流程记录</div><div class="timeline"><div class="timeline-item"><div class="timeline-title">${t.createdAt || "2026-08-01 09:00"} · 任务已生成</div><div class="timeline-content">执行人：${t.pm}</div></div>${t.everOverdue ? `<div class="timeline-item"><div class="timeline-title">${t.firstOverdueAt || addDays(t.due, 1)} · 首次转为当前逾期</div><div class="timeline-content">原截止日期：${t.originalDue || t.due}</div></div>` : ""}${pauseTimeline}${resumeTimeline}${t.status === "done" ? `<div class="timeline-item"><div class="timeline-title">${record?.createdAt || t.completedAt || DEMO_TODAY} · 任务已完成</div><div class="timeline-content">${completionTypeName(t.completionType)}；维系记录已保存</div></div>` : ""}${t.status === "cancelled" ? `<div class="timeline-item"><div class="timeline-title">${escapeHtml(t.closedAt || DEMO_TODAY)} · ${autoClosed ? "任务已关闭" : "任务已取消"}</div><div class="timeline-content">${escapeHtml(t.closeReason || (autoClosed ? "系统自动关闭" : "系统取消"))}${t.employeeStopRecordId ? `；关联员工停用 ${escapeHtml(t.employeeStopRecordId)}` : ""}</div></div>` : ""}</div></div><div class="drawer-foot"><button class="btn" data-close>关闭</button>${canChange ? `<button class="btn" data-action="change-task" data-id="${t.id}">${changeActionLabel}</button>` : ""}${canExecute ? `<button class="btn btn-primary" data-complete="${t.id}">${currentUser.fullAccess ? "管理员代办完成" : "提交维系结果"}</button>` : ""}</div>`,
        );
      }
      ...
      function openTaskChange(id) {
        ...
        const refreshChangeFields = () => {
          ...
          $("#changeDateHint").textContent = isDelay
            ...
            : "须晚于当前日期，最长暂停 365 日；暂停期间不计逾期，不影响健康状态";
      ...
      function openCampaign(id) {
        ...
        const refreshCampaignFields = () => {
          ...
          if (isCoverage) {
            ...
            $("#camPreview").innerHTML = `预计匹配 ${preview.people.length} 名有效关键人，涉及 ${preview.owners.length} 名执行人。`;
          }
        };
