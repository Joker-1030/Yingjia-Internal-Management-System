      ...
      function personDetailCanMaintain(person) {
        return !person.archived && canMaintainContact(person);
      }
          ...
          const scopedTaskRows =
            selectedPersonTaskScope === "all"
              ...
              : selectedPersonTaskScope === "done"
                ? finishedTasks
                : activeTasks;
          content = `<div class="metrics compact-metrics">${metric("当前逾期", taskCounts.overdue, "", "red")}${metric("今日到期", taskCounts.today, "待执行", "orange")}${metric("未来 7 天", taskCounts.next7, "待执行", "yellow")}${metric("其他待执行", taskCounts.pending, "七天以后")}${metric("暂停中", taskCounts.paused, "", "blue")}</div><div class="tabs execution-tabs"><button class="tab ${selectedPersonTaskScope === "unfinished" ? "active" : ""}" type="button" data-person-task-scope="unfinished">未结束 <span class="tab-count">${activeTasks.length}</span></button><button class="tab ${selectedPersonTaskScope === "all" ? "active" : ""}" type="button" data-person-task-scope="all">全部 <span class="tab-count">${personTasks.length}</span></button><button class="tab ${selectedPersonTaskScope === "done" ? "active" : ""}" type="button" data-person-task-scope="done">已完成 <span class="tab-count">${finishedTasks.length}</span></button></div><div class="table-wrap"><table style="min-width:1200px"><thead><tr><th>任务执行记录编号</th><th>客户公司</th><th>任务类型</th><th>任务标题</th><th>状态</th><th>截止时间</th><th>逾期天数</th><th>执行人</th><th>完成认定</th><th>操作</th></tr></thead><tbody>${taskRows(scopedTaskRows) || '<tr><td colspan="10">当前范围暂无任务</td></tr>'}</tbody></table></div>`;
        }
        if (selectedPersonDetailTab === "records")
          content = `<div class="table-wrap"><table><thead><tr><th>记录编号</th><th>维系时间</th><th>方式</th><th>沟通摘要</th><th>维系人</th><th>关联任务</th><th>创建 / 更新时间</th><th>操作</th></tr></thead><tbody>${records.map((record) => { const linkedTask = tasks.find((task) => task.id === record.taskId); return `<tr><td>${maintenanceRecordCode(record)}</td><td>${record.maintenanceAt || record.date}</td><td>${record.method}${record.method === "其他" ? ` · ${escapeMaintenanceMethodText(record.otherMethod || "未说明")}` : ""}</td><td>${record.summary}</td><td>${record.pm}</td><td>${linkedTask?.executionCode || "未关联"}</td><td>${record.createdAt || record.date}<div class="list-sub">更新 ${record.updatedAt || record.createdAt || record.date}</div></td><td><button class="link" data-action="record-detail" data-id="${record.id}">详情</button></td></tr>`; }).join("") || '<tr><td colspan="8">暂无维系记录</td></tr>'}</tbody></table></div>`;
        if (selectedPersonDetailTab === "logs")
          content = '<div class="empty">暂无操作日志</div>';
        const html = `<div class="drawer-head"><div class="modal-title">关键人详情</div><button class="icon-btn close" data-close>×</button></div><div class="drawer-body"><div class="detail-hero"><div class="avatar">${person.name[0]}</div><div><div class="detail-name">${person.name} ${person.archived ? '<span class="tag red">已停用</span>' : contactHasOverdue(person) ? '<span class="tag red">当前逾期</span>' : '<span class="tag green">健康</span>'}</div></div></div><div class="tabs detail-tabs">${tabs.map(([key, label]) => `<button class="tab ${selectedPersonDetailTab === key ? "active" : ""}" type="button" data-person-detail-tab="${key}">${label}</button>`).join("")}</div><div class="person-detail-content">${content}</div></div><div class="drawer-foot"><button class="btn" data-close>关闭</button>${objectLifecycleActionHtml("contact", person.id)}${personDetailCanMaintain(person) ? `<button class="btn" data-action="edit-contact" data-id="${person.id}">编辑身份</button>` : ""}${!person.archived && canTransferContact(person) ? `<button class="btn" data-action="transfer" data-id="${person.id}">调岗</button>` : ""}${!person.archived && canCreateMaintenanceForPerson(person) ? `<button class="btn btn-primary" data-action="new-record" data-id="${person.id}">新增记录</button>` : ""}</div>`;
        if (initial)
      ...
      function openPerson(id) {
        ...
        const canViewStoppedContact = Boolean(
          currentUser?.fullAccess && person?.archived && companyIsVisible(company),
        );
        if (
          !person ||
          (!canViewStoppedContact &&
            !scopedContacts().some((item) => item.id === id))
        )
      ...
      function genericAction(action, id, kind) {
        const actions = {
          ...
          "view-department-associations": () => openDepartmentAssociationDetails(String(id)),
          ...
          "view-position-associations": () => openPositionAssociationDetails(String(id)),
      ...
      function activeObjectArchiveRecord(kind, id) {
        return archivedItems.find(
          (item) =>
            ...
            (kind === "group"
              ? item.name === String(id)
              : String(item.targetId) === String(id)),
        );
      }

      function objectLifecycleActionHtml(kind, id) {
        const archived = activeObjectArchiveRecord(kind, id);
        if (archived)
          return currentUser?.fullAccess
            ? `<button class="btn btn-primary" data-action="restore-object" data-id="${archived.id}">恢复</button>`
            : "";
        return stopObjectActionHtml(kind, id);
      }
      ...
      function openStopObject(kind, id) {
        ...
        $("#stopForm").onsubmit = (e) => {
          ...
          closeOverlay();
          renderPage();
          toast(kind === "contact" ? "停用成功" : "停用已直接生效并记录操作历史");
        };
      }
