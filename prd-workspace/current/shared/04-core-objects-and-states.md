...
| 对象 | 状态 | 说明 |
| --- | --- | --- |
| 员工 | 在职、停用 | HR/admin 停用或恢复不创建审批；有`已立项`、`进行中`或`已交付`项目时先通过 M08 既有地区责任调整完成项目迁移，否则阻止停用；`已完成`继续迁移，`已取消`和`已中止`不迁移且不阻断停用；停用禁止登录，历史工号永久保留，本期无离职状态 |
...
| 审批实例 | 审批中、已通过、已驳回、已通过-业务处理失败 | 普通审批动作仅提交、通过、驳回；仍属于 M04 的业务处理失败不回退审批意见并后台幂等重试；客户侧停用/恢复不创建审批实例或使用该重试链路 |
...
- 暂停只适用于当前未逾期且从未逾期的常规维系、生日关怀和节假日关怀；申请提交和审批生效时均须校验，专项维系和覆盖 KPI 不允许暂停。暂停期间仍归“待执行 / 暂停”，不形成当前逾期、曾经逾期、首次逾期时间或健康风险。常规维系恢复时按恢复日、关键人当前职级和当前有效周期重算当前截止；生日/节假日于截止前恢复时沿用原当前截止，暂停越过截止时自动关闭并退出有效集合。详见 `DEC-200《暂停任务不形成逾期并按任务类型恢复或关闭》`：[`prd-workspace/decisions/DEC-200-暂停任务不形成逾期并按任务类型恢复或关闭.md`](../../decisions/DEC-200-暂停任务不形成逾期并按任务类型恢复或关闭.md) → “决策”“暂停期间”“常规维系恢复”“生日/节假日恢复或关闭”。
