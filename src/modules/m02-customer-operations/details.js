      ...
      function openStopObject(kind, id) {
        ...
        const replacementDepartments =
          kind === "department"
            ...
            : [];
        openModal(
          `<div class="modal-head"><div class="modal-title">停用${kindName}</div><button class="icon-btn close" data-close>×</button></div><form id="stopForm"><div class="modal-body"><div class="detail-grid"><div class="detail-item"><label>对象</label><div>${obj.name}</div></div>${kind === "group" ? `<div class="detail-item"><label>集团编号</label><div>${customerGroupNumbers[obj.name]}</div></div>` : ""}<div class="detail-item"><label>当前状态</label><div>正常</div></div>${kind === "group" ? `<div class="detail-item"><label>正常客户单位</label><div>${affectedCustomers.length}</div></div><div class="detail-item"><label>正常客户部门</label><div>${customerDepartments.filter((item) => !item.archived && item.group === obj.name).length}</div></div>` : ""}${kind !== "contact" ? `<div class="detail-item"><label>有效关键人 / 任职</label><div>${affectedPeople.length}</div></div>` : ""}${kind !== "contact" ? `<div class="detail-item"><label>未完成任务</label><div>${unfinishedTasks.length}</div></div><div class="detail-item"><label>覆盖 KPI 影响</label><div>${affectedPeople.length ? "将实时重算" : "无"}</div></div><div class="detail-item"><label>未完成任务处理</label><div>统一关闭并记录原因</div></div>` : ""}</div>${kind === "group" && affectedCustomers.length ? `<label class="choice-item" style="margin-top:var(--space-4)"><input id="stopCascadeGroup" type="checkbox"><span>完整级联停用全部 ${affectedCustomers.length} 家客户单位及 ${affectedPeople.length} 名有效关键人</span></label>` : ""}${kind === "customer" && affectedPeople.length ? `<label class="choice-item" style="margin-top:var(--space-4)"><input id="stopCascade" type="checkbox"><span>完整级联停用该单位全部 ${affectedPeople.length} 名有效关键人</span></label>` : ""}${kind === "department" && affectedPeople.length ? `<div class="form-group"><label class="form-label"><span class="required-marker" aria-hidden="true">*</span>替代客户部门</label><select class="input" id="stopReplacementDepartment"><option value="">请选择同一客户公司内的正常部门</option>${replacementDepartments.map((department) => `<option value="${department.id}">${department.name}</option>`).join("")}</select><div class="list-sub">仅迁移当前任职，历史任职保留原部门名称</div></div>` : ""}<div class="form-group"><label class="form-label"><span class="required-marker" aria-hidden="true">*</span>停用原因</label><textarea class="input" id="stopReason" minlength="5" maxlength="500" required placeholder="请填写 5-500 字停用原因"></textarea></div><div class="role-note danger-note">${kind === "contact" ? `停用后，${unfinishedTasks.length} 条未完成任务将关闭。` : "确认后立即停用，不生成审批流程。对象将从正常列表、候选和实时统计中剔除，适用未完成任务统一关闭并保留历史；后续可由 admin 按当前规则恢复。"}</div></div><div class="modal-foot"><button class="btn" type="button" data-close>取消</button><button class="btn btn-danger" type="submit">确认停用</button></div></form>`,
        );
        $("#stopForm").onsubmit = (e) => {
          e.preventDefault();
