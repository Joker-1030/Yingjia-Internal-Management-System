      function openTaskDetail(id) {
        ...
        const completionAudit =
          t.status === "done" || t.status === "late_entry_pending"
            ...
            : "";
        const employeeStopCancellation =
          t.status === "cancelled" &&
          t.cancellationSource === "employee_deactivation"
            ? `<div class="detail-item"><label>取消前状态</label><div>${escapeHtml(taskStatusName(t.statusBeforeCancellation, t))}</div></div><div class="detail-item"><label>实际操作人</label><div>${escapeHtml(t.cancelledBy || "—")}</div></div><div class="detail-item full"><label>关联员工停用</label><div>${escapeHtml(t.employeeStopEmployeeName || t.pm)} · ${escapeHtml(t.employeeStopRecordId || "—")}</div></div>`
            : "";
        openDrawer(
          `<div class="drawer-head"><div class="modal-title">任务详情</div><button class="icon-btn close" data-close>×</button></div><div class="drawer-body"><div class="detail-hero"><div class="avatar">任</div><div><div class="detail-name">${t.title}</div><div class="detail-sub">${taskDisplayType(t)} · ${taskStatusName(t.status, t)}</div></div></div><div class="detail-grid"><div class="detail-item"><label>任务编号</label><div>${t.parentTaskCode}</div></div><div class="detail-item"><label>任务执行记录编号</label><div>${t.executionCode}</div></div><div class="detail-item full"><label>任务标题</label><div>${t.title}</div></div><div class="detail-item"><label>关键人 / 覆盖目标</label><div>${t.person}</div></div><div class="detail-item"><label>执行人</label><div>${t.pm} · ${t.executorRole || (t.company?.includes("有限公司") ? "区域总监" : "PM")}</div></div><div class="detail-item"><label>客户单位 / 集合</label><div>${t.company}</div></div><div class="detail-item"><label>任务类型</label><div>${t.type}</div></div><div class="detail-item"><label>生成时间</label><div>${t.createdAt || "2026-08-01 09:00"}</div></div><div class="detail-item"><label>更新时间</label><div>${t.updatedAt || t.createdAt || "-"}</div></div><div class="detail-item"><label>原截止时间</label><div>${t.originalDue || t.due} 23:59:59</div></div><div class="detail-item"><label>当前截止时间</label><div>${t.due} 23:59:59</div></div><div class="detail-item full"><label>执行要求</label><div>${taskRequirementText(t)}</div></div><div class="detail-item full"><label>补完成策略</label><div>${completionPolicy.allowed ? completionPolicy.cutoff ? `允许补完成至 ${completionPolicy.cutoff} 23:59:59` : "允许逾期补完成且无截止日期" : "不允许逾期补完成"}</div></div>${t.type === "常规维系" ? '<div class="detail-item full"><label>逾期规则</label><div>超过当前截止后持续保持“当前逾期”，无补完成截止；直至逾期补完成或受控关闭</div></div>' : ""}${t.resumeDate ? `<div class="detail-item"><label>暂停至</label><div>${t.resumeDate}</div></div>` : ""}${taskIsHealthRisk(t) && t.status === "paused" ? '<div class="detail-item"><label>健康影响</label><div><span class="tag red">暂停中已超当前截止，仍计入健康风险</span></div></div>' : ""}${hasPendingChange ? '<div class="detail-item full"><label>任务变更</label><div><span class="tag yellow">存在审批中的延期、暂停或取消申请，不能重复发起</span></div></div>' : ""}${t.closeReason ? `<div class="detail-item"><label>取消原因</label><div>${escapeHtml(t.closeReason)}</div></div>` : ""}${employeeStopCancellation}</div>${completionAudit}<div class="section-title">流程记录</div><div class="timeline"><div class="timeline-item"><div class="timeline-title">${t.createdAt || "2026-08-01 09:00"} · 任务已生成</div><div class="timeline-content">系统根据${taskDisplayType(t)}规则生成并分配给${t.pm}</div></div>${t.everOverdue ? `<div class="timeline-item"><div class="timeline-title">${t.firstOverdueAt || addDays(t.due, 1)} · 首次转为当前逾期</div><div class="timeline-content">原截止日期 ${t.originalDue || t.due} 保留，后续完成不清除曾经逾期事实</div></div>` : ""}${t.status === "late_entry_pending" ? `<div class="timeline-item"><div class="timeline-title">${pendingRecord?.createdAt || recordCreatedAt()} · 已提交逾期补录审批</div><div class="timeline-content">实际维系日期 ${pendingRecord?.date || "待核对"}，按当前截止时间核验，等待${taskApprovalRoute(t).current}</div></div>` : ""}${t.status === "paused" ? `<div class="timeline-item"><div class="timeline-title">${t.pausedAt || "2026-08-10 10:00"} · 暂停审批已通过</div><div class="timeline-content">${t.resumeDate} 自动恢复；暂停期间超过当前截止仍计入健康风险</div></div>` : ""}${t.status === "done" ? `<div class="timeline-item"><div class="timeline-title">${record?.createdAt || t.completedAt || DEMO_TODAY} · 任务已完成</div><div class="timeline-content">${completionTypeName(t.completionType)}；已生成维系记录${t.type === "常规维系" ? "并按实际维系日续期" : ""}</div></div>` : ""}${t.status === "cancelled" ? `<div class="timeline-item"><div class="timeline-title">${escapeHtml(t.closedAt || DEMO_TODAY)} · 任务已取消</div><div class="timeline-content">${escapeHtml(t.closeReason || "系统取消")}${t.employeeStopRecordId ? `；关联员工停用 ${escapeHtml(t.employeeStopRecordId)}` : ""}</div></div>` : ""}</div></div><div class="drawer-foot"><button class="btn" data-close>关闭详情链</button>${canChange && !hasPendingChange ? `<button class="btn" data-action="change-task" data-id="${t.id}">延期/取消/暂停</button>` : ""}${canExecute ? `<button class="btn btn-primary" data-complete="${t.id}">${currentUser.fullAccess ? "管理员代办完成" : "提交维系结果"}</button>` : ""}</div>`,
        );
      }
