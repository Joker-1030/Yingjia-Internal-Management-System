      ...
      function openArchiveAudit(id) {
        ...
        openDrawer(
          `<div class="drawer-head"><div><div class="modal-title">停用记录详情</div><div class="panel-sub">${x.flowCode} · ${x.type}</div></div><button class="icon-btn close" data-close>×</button></div><div class="drawer-body"><div class="detail-hero"><div class="avatar">停</div><div><div class="detail-name">${x.name}</div><div class="detail-sub">${x.parent} · ${x.region}</div></div><div class="spacer"></div><span class="tag ${x.status === "已停用" ? "red" : "yellow"}">${x.status}</span></div><div class="detail-grid"><div class="detail-item"><label>对象快照</label><div>${x.type} · ${x.name}</div></div><div class="detail-item"><label>停用前状态</label><div>正常</div></div><div class="detail-item"><label>停用原因</label><div>${x.reason}</div></div><div class="detail-item"><label>任务处理方式</label><div>${x.taskHandle}</div></div><div class="detail-item"><label>影响摘要</label><div>${x.impact}</div></div><div class="detail-item"><label>停用生效时间</label><div>${x.effectiveAt || "—（审批通过后生成）"}</div></div><div class="detail-item"><label>停用审批状态</label><div>${x.approvalStatus || "已通过"}</div></div><div class="detail-item"><label>恢复审批</label><div>${x.recoveryStatus || "未申请"}</div></div></div>${x.upperDisabled ? `<div class="role-note danger-note">恢复阻断：${x.upperDisabledReason}</div>` : ""}<div class="section-title">审批与生效记录</div><div class="timeline"><div class="timeline-item is-done"><div class="timeline-title">${x.date} · ${x.applicant} 发起停用申请</div><div class="timeline-content">流程 ${x.flowCode}；原因：${x.reason}</div></div>${stopTimeline}${x.status === "恢复审批中" ? `<div class="timeline-item is-current"><div class="timeline-title">恢复审批中</div><div class="timeline-content">${x.recoveryStatus}；当前处理人 ${approvalCurrentAssignees(recoveryApproval || {}).join("、") || "待系统计算"}</div></div>` : ""}</div></div><div class="drawer-foot"><button class="btn" data-close>关闭</button>${stopApproval ? `<button class="btn" data-action="approval-detail" data-id="${stopApproval.id}">查看停用审批</button>` : ""}${recoveryApproval ? `<button class="btn" data-action="approval-detail" data-id="${recoveryApproval.id}">查看恢复审批</button>` : ""}${x.status === "已停用" && hasOperationPermission("archive.restore") ? `<button class="btn btn-primary" data-action="restore-object" data-id="${x.id}">申请恢复</button>` : ""}</div>`,
        );
      }
