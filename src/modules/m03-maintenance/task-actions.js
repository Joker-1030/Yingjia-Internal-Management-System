      ...
      function openCompleteTask(id) {
        ...
        const refreshRecognition = () => {
          ...
          $("#completeSubmit").textContent = lateEntry
            ...
            : currentUser.fullAccess
              ...
              : "提交并完成任务";
          if (lateEntry) {
            $("#completionRecognition").innerHTML = `<strong>认定：按期执行、逾期补录。</strong>提交后完成任务。`;
            ...
            $("#completionRecognition").innerHTML = blockedReason
              ...
              : `<strong>认定：逾期补完成。</strong>计入总完成率，不计入按期完成率。`;
          } else {
        ...
        $("#completeForm").onsubmit = (e) => {
          e.preventDefault();
          const otherField = $("#completeOtherMethod");
          const otherMethod = otherField.value.trim();
          if ($("#completeMethod").value === "其他" && (otherMethod.length < 2 || otherMethod.length > 30)) {
            otherField.setCustomValidity("请填写 2-30 字其他方式说明");
          ...
          const record = {
            ...
            otherMethod: $("#completeMethod").value === "其他" ? otherMethod : "",
            date,
            createdAt,
