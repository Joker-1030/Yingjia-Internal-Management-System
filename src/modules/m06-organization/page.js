        ...
        const item =
          ...
          (personnelItem
            ? {
                ...
                detail: `部门：${employeeDepartmentNames(employeeByCode(personnelItem.employeeCode)).join("、") || personnelItem.toDept}；系统角色：${(personnelItem.toRoles || []).join("、")}；${personnelItem.impactSummary || "直接生效，无审批或交接"}；原因：${personnelItem.reason}`,
      ...
      function renderEmployees() {
        ...
        const departmentSummary = selectedDepartment
          ? `<div class="company-overview" style="margin:0 0 12px"><div class="overview-item"><label>当前部门</label><div>${departmentPath(selectedDepartment)}</div></div><div class="overview-item"><label>部门编码</label><div>${selectedDepartment.code}</div></div><div class="overview-item"><label>部门主管</label><div>${selectedSupervisor?.name || "待设置"}${selectedSupervisor ? ` · ${selectedSupervisor.code}` : ""}</div></div><div class="overview-item"><label>组织类型</label><div>${selectedDepartment.type === "region" ? "区域中心" : "普通部门"}</div></div></div>`
          : "";
        const directory = `<section class="panel" style="margin-top:14px"><div class="toolbar employee-toolbar"><input class="input" id="employeeSearch" maxlength="100" placeholder="搜索姓名、工号或手机号后四位"><select class="input" id="employeeRole"><option value="">全部系统角色</option>${["总裁", "市场副总", "区域总监", "PM", "HR/人事"].map((role) => `<option>${role}</option>`).join("")}</select><select class="input" id="employeeStatus"><option value="">全部员工状态</option><option ${dashboardEmployeeStatusFilter === "在职" ? "selected" : ""}>在职</option>${currentUser.fullAccess || currentUser.role === "hr" ? `<option ${dashboardEmployeeStatusFilter === "停用" ? "selected" : ""}>停用</option>` : ""}</select>${currentUser.fullAccess ? `<select class="input" id="employeeAccountStatus"><option value="">全部账号状态</option><option>启用</option><option>停用</option></select>` : ""}<span class="spacer"></span><span class="panel-sub employee-count">共 ${rows.length} 条部门成员关系 / ${visibleEmployees.length} 名员工</span></div><div class="split-view"><div class="tree"><div class="tree-node active" data-dept-id="" data-dept="">▦ 英嘉科技</div>${organizationTreeRows()}</div><div class="table-wrap"><table style="min-width:980px"><thead><tr><th>员工</th><th>工号</th><th>当前部门行</th><th>全部部门</th><th>系统角色</th><th>手机号</th><th>员工状态</th>${currentUser.fullAccess ? "<th>账号状态</th><th>最近登录</th>" : ""}<th>操作</th></tr></thead><tbody id="employeeBody">${rows
          .map(({ employee: e, departmentName }) => {
            ...
            return `<tr data-dept-name="${departmentName}" data-role="${roleNames.join("|") || "未关联业务角色"}" data-status="${e.status}" data-account-status="${e.accountStatus}" data-search="${e.name}${e.code}${phone.slice(-4)}"><td><div class="person"><div class="avatar">${e.name[0]}</div><strong>${e.name}</strong>${department?.supervisorCode === e.code ? ' <span class="tag">主管</span>' : ""}</div></td><td><span class="tag">${e.code}</span></td><td>${departmentPath(department)}</td><td>${employeeDepartmentNames(e).map((name) => `<span class="tag">${name}</span>`).join(" ") || "—"}</td><td>${roleNames.map((role) => `<span class="tag blue">${role}</span>`).join(" ") || '<span class="tag">未关联</span>'}</td><td>${phone}</td><td><span class="tag ${e.status === "在职" ? "green" : "yellow"}">${e.status}</span></td>${currentUser.fullAccess ? `<td><span class="tag ${e.accountStatus === "启用" ? "green" : "red"}">${e.accountStatus}</span></td>` : ""}${currentUser.fullAccess ? `<td>${e.lastLogin}</td>` : ""}<td><button class="link" data-action="employee-detail" data-id="${i}">详情</button>${e.role !== "系统管理员" && canEmployeeAction("employees.edit_employee") && e.name !== currentUser.name ? ` · <button class="link" data-action="employee-edit" data-id="${i}">编辑员工</button>` : ""}${e.role !== "系统管理员" && canEmployeeAction("employees.suspend_employee") && e.name !== currentUser.name && e.status === "在职" ? ` · <button class="link" data-action="employee-suspend" data-id="${i}">停用</button>` : ""}${e.role !== "系统管理员" && canEmployeeAction("employees.restore_employee") && e.status === "停用" ? ` · <button class="link" data-action="employee-restore" data-id="${i}">恢复</button>` : ""}${e.role !== "系统管理员" && canEmployeeAction("employees.reset_password") && e.name !== currentUser.name ? ` · <button class="link" data-action="reset-password" data-id="${i}">重置密码</button>` : ""}</td></tr>`;
          })
        ...
        return (
          ...
          (!canViewDirectory && !canViewChanges
            ? '<section class="panel" style="margin-top:14px"><div class="empty">当前角色没有组织明细或变动记录操作权限</div></section>'
            : employeeView === "directory"
              ? departmentSummary + directory
              : changePanel)
        );
      }
