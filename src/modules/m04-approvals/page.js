      ...
      function canWithdrawApproval(approval) {
        return Boolean(
          approval.status === "pending" &&
            hasOperationPermission("approvals.withdraw") &&
            approval.applicant === currentUser.name &&
            !approval.acceptedBy &&
            !approval.rejectedBy &&
            !approval.decidedBy &&
            !(approval.handledBy || []).length &&
            !(approval.collaborationNodes || []).some((node) =>
              node.members?.some((member) =>
                ["done", "rejected"].includes(member.state),
              ),
            ),
        );
      }

      function canCollaborateOnApproval(approval, operation) {
        return Boolean(
          approval.status === "pending" &&
      ...
      function canAppendApprovalCc(approval) {
        return Boolean(
          approval.status === "pending" &&
            hasOperationPermission("approvals.append_cc") &&
            (currentUser.fullAccess ||
              approval.applicant === currentUser.name ||
              approvalCurrentAssignees(approval).includes(currentUser.name)),
        );
      ...
      function bindApprovalFilters() {
        ...
        controls.forEach(
          (control) => (control.oninput = control.onchange = apply),
        );
