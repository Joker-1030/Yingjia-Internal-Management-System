      function renderTasks() {
        ...
        host.innerHTML = html;
        const taskSearch = host.querySelector("#taskSearch");
        if (taskSearch)
          taskSearch.placeholder = "搜索任务编号、执行编号、任务、关键人、客户";
      ...
      function renderTaskSummary(ts) {
        ...
        return `<div class="toolbar" id="taskSummaryFilters" style="flex-wrap:wrap"><input class="input" id="summarySearch" placeholder="搜索任务编号、任务名称、范围"><select class="input" id="summaryType"><option value="">全部任务类型</option><option>常规维系</option><option>专项维系</option><option>关键人覆盖 KPI</option><option>生日关怀</option><option>节假日关怀</option><option value="care">生日/节日关怀</option></select><select class="input" id="summaryStatus"><option value="">全部主题状态</option><option>进行中</option><option>待开始</option><option>已过期</option><option>已完成</option></select><select class="input" id="summaryOwner"><option value="">全部执行人</option>${summaryOwnerOptions.map((name) => `<option>${name}</option>`).join("")}</select><button class="btn" id="resetSummaryFilters" type="button">重置</button><span class="spacer"></span><span class="panel-sub" id="summaryFilterCount">共 ${rows.length} 项任务</span></div><div class="table-wrap"><table style="min-width:1540px"><thead><tr><th>任务 / 编号</th><th>任务类型</th><th>任务状态</th><th>目标范围 / 执行周期</th><th>覆盖客户</th><th>覆盖关键人</th><th>已完成</th><th>待执行/暂停</th><th>当前逾期</th><th>补录审核中</th><th>已过期未完成</th><th>任务完成进度</th><th>更新时间</th><th>操作</th></tr></thead><tbody id="taskSummaryBody">${rows.map((row) => {
          ...
          return `<tr data-summary-search="${row.taskCode}${row.name}${row.scope}${row.period}" data-summary-type="${row.type}" data-summary-status="${row.themeStatus}" data-summary-owner="${owners}"><td><strong>${row.name}</strong><div class="list-sub">任务编号 ${row.taskCode || "待生成"}</div></td><td><span class="tag ${taskTypeMeta(row.type).tone}">${row.type}</span></td><td>${taskThemeStatusTag(row.themeStatus)}</td><td>${row.scope}<div class="list-sub">${row.period}</div></td><td>${row.customers}</td><td><strong>${row.contacts}</strong></td><td><span class="tag green">${row.done}</span></td><td>${row.pending}</td><td><span class="tag ${row.overdue ? "red" : "green"}">${row.overdue}</span></td><td>${row.lateEntryPending}</td><td>${row.expired}</td><td style="min-width:150px"><div style="display:flex;justify-content:space-between;margin-bottom:5px"><span>${row.done}/${row.total}</span><strong>${row.rate}%</strong></div><div class="progress"><i style="width:${row.rate}%"></i></div></td><td>${row.updatedAt || taskDataUpdatedAt}</td><td><span class="link" data-action="${row.campaignId ? "campaign-detail" : "task-theme-detail"}" data-id="${row.campaignId || row.key}">详情</span></td></tr>`;
      ...
      function renderTaskExecutions(ts) {
        ...
        return `<div class="toolbar" id="taskExecutionFilters" style="flex-wrap:wrap"><input class="input" id="taskSearch" placeholder="搜索任务、关键人、客户"><select class="input" id="taskType"><option value="">全部类型</option><option>常规维系</option><option>专项维系</option><option>关键人覆盖 KPI</option><option>生日关怀</option><option>节假日关怀</option><option value="care">生日/节日关怀</option></select><select class="input" id="taskOwner"><option value="">全部执行人</option>${ownerOptions.map((name) => `<option>${name}</option>`).join("")}</select><select class="input" id="taskRegion"><option value="">全部区域</option>${regionOptions.map((name) => `<option>${name}</option>`).join("")}</select><select class="input" id="taskCityFilter"><option value="">全部城市</option>${cityOptions.map((name) => `<option>${name}</option>`).join("")}</select><select class="input" id="taskRiskScope"><option value="">全部风险范围</option><option value="risk">风险任务</option></select><select class="input" id="taskEventType"><option value="">全部业务事件</option><option value="done">完成事件</option><option value="overdue">首次逾期事件</option></select><input class="input" id="taskEventMonth" type="month" title="业务事件月份"><select class="input" id="taskCompletionType"><option value="">全部完成认定</option><option value="on-time">按期完成</option><option value="late-entry">逾期补录</option><option value="late-completion">逾期补完成</option></select><label class="panel-sub">截止日期</label><input class="input" id="taskDueStart" type="date" title="截止日期起"><span class="panel-sub">至</span><input class="input" id="taskDueEnd" type="date" title="截止日期止"><button class="btn" id="resetTaskFilters" type="button">重置</button><span class="spacer"></span><span class="panel-sub" id="taskFilterCount">共 ${active.length} 条执行明细</span></div>${taskUpdateFooter("refresh-execution-list", "main")}<div class="execution-table" data-execution-table="main-executions"><div class="tabs execution-tabs">${executionStatusTabsHtml(counts, preferredStatus)}</div><div class="table-wrap"><table><thead><tr><th>执行任务</th><th>类型</th><th>关键人</th><th>区域</th><th>城市</th><th>执行人</th><th>截止日</th><th>状态</th><th>操作</th></tr></thead><tbody id="taskBody">${active.map((t) => {
      ...
      function renderTaskRecords() {
        ...
        return `<div class="toolbar" id="recordFilters" style="flex-wrap:wrap"><input class="input" id="recordSearch" maxlength="100" placeholder="记录编号 / 关键人 / 客户 / 摘要"><select class="input" id="recordMethod"><option value="">全部方式</option><option>电话</option><option>微信</option><option>线下拜访</option><option>视频会议</option><option>邮件</option><option>其他</option></select><select class="input" id="recordExecutor"><option value="">全部维系人</option>${executorOptions.map((name) => `<option>${name}</option>`).join("")}</select><select class="input" id="recordRegion"><option value="">全部区域</option>${regionOptions.map((name) => `<option>${name}</option>`).join("")}</select><select class="input" id="recordCity"><option value="">全部城市</option>${cityOptions.map((name) => `<option>${name}</option>`).join("")}</select><select class="input" id="recordLinkedTask"><option value="">是否关联任务</option><option value="yes">已关联任务</option><option value="no">未关联任务</option></select><select class="input" id="recordAttachment"><option value="">是否有附件</option><option value="yes">有附件</option><option value="no">无附件</option></select><label class="panel-sub">维系日期</label><input class="input" id="recordDateStart" type="date" title="维系日期起"><span class="panel-sub">至</span><input class="input" id="recordDateEnd" type="date" title="维系日期止"><label class="panel-sub">创建日期</label><input class="input" id="recordCreatedStart" type="date" title="创建日期起"><span class="panel-sub">至</span><input class="input" id="recordCreatedEnd" type="date" title="创建日期止"><button class="btn" id="resetRecordFilters" type="button">重置</button><span class="spacer"></span><span class="panel-sub" id="recordFilterCount">共 ${records.length} 条 · 数据范围：${currentUser.region}</span></div><div class="table-wrap"><table style="min-width:1740px"><thead><tr><th>记录编号</th><th>维系时间</th><th>关键人</th><th>客户单位</th><th>维系方式</th><th>沟通摘要</th><th>维系人</th><th>录入人</th><th>关联任务</th><th>附件数</th><th>创建时间</th><th>更新时间</th><th>操作</th></tr></thead><tbody id="recordBody">${records.map((r) => { const linkedTask = tasks.find((task) => task.id === r.taskId); const code = maintenanceRecordCode(r); return `<tr data-search="${code}${r.person}${r.company}${r.method}${r.otherMethod || ""}${r.summary}${r.pm}${r.createdBy || r.pm}${r.proxyReason || ""}" data-record-method="${r.method}" data-record-executor="${r.pm}" data-record-region="${r.region}" data-record-city="${recordCity(r)}" data-record-date="${r.date}" data-record-created="${(r.createdAt || r.date).slice(0, 10)}" data-record-linked="${r.taskId ? "yes" : "no"}" data-record-attachment="${r.attachments.length ? "yes" : "no"}"><td><strong>${code}</strong></td><td>${r.maintenanceAt || r.date}</td><td><strong>${r.person}</strong></td><td>${r.company}</td><td><span class="tag blue">${r.method}</span>${r.method === "其他" ? `<div class="list-sub">${r.otherMethod || "未说明"}</div>` : ""}</td><td style="max-width:320px">${r.summary}</td><td>${r.pm}</td><td>${r.createdBy || r.pm}${r.proxyOperator ? ' <span class="tag orange">代办</span>' : ""}</td><td>${linkedTask ? `<button class="link" data-action="task-detail" data-id="${linkedTask.id}">${linkedTask.executionCode}</button>` : "未关联任务"}</td><td>${r.attachments.length ? `<button class="link" data-action="preview-file" title="点击预览附件">${r.attachments.length}</button>` : "0"}</td><td>${r.createdAt || r.date}</td><td>${r.updatedAt || r.createdAt || r.date}</td><td><button class="link" data-action="record-detail" data-id="${r.id}">详情</button>${canEditMaintenanceRecord(r) ? ` · <button class="link" data-action="edit-record" data-id="${r.id}">编辑</button>` : ""}</td></tr>`; }).join("") || '<tr data-empty-row><td colspan="13">当前范围暂无维系记录</td></tr>'}</tbody></table></div>`;
      }
