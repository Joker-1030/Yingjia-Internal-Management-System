      function openRecord(personId, recordId) {
        ...
        openModal(
          `<div class="modal-head"><div class="modal-title">${r ? "编辑" : "新增"}维系记录</div><button class="icon-btn close" data-close>×</button></div><form id="recordForm"><div class="modal-body"><div class="form-grid"><div class="form-group full"><label class="form-label">关键人 *</label>${
            lockedPerson
        ...
        $("#recordForm").onsubmit = (e) => {
          ...
          else if (linkedTask?.status === "overdue" && data.date <= linkedTask.due) {
      ...
      function openRecordDetail(id) {
        ...
        openDrawer(
          `<div class="drawer-head"><div class="modal-title">维系记录详情</div><button class="icon-btn close" data-close>×</button></div><div class="drawer-body"><div class="detail-hero"><div class="avatar">${r.person[0]}</div><div><div class="detail-name">${r.person}</div><div class="detail-sub">${maintenanceRecordCode(r)} · ${r.company} · ${r.method}${r.method === "其他" ? `（${r.otherMethod || "未说明"}）` : ""}</div></div></div><div class="detail-grid"><div class="detail-item"><label>记录编号</label><div>${maintenanceRecordCode(r)}</div></div><div class="detail-item"><label>记录状态</label><div><span class="tag green">正式生效</span></div></div><div class="detail-item"><label>维系时间</label><div>${r.maintenanceAt || r.date}</div></div><div class="detail-item"><label>创建时间</label><div>${r.createdAt || "—"}</div></div><div class="detail-item"><label>更新时间</label><div>${r.updatedAt || r.createdAt || "—"}</div></div><div class="detail-item"><label>业务维系人</label><div>${r.pm}</div></div><div class="detail-item"><label>实际录入人</label><div>${r.createdBy || r.pm}${r.proxyOperator ? ' <span class="tag orange">管理员代办</span>' : ""}</div></div><div class="detail-item"><label>关联任务</label><div>${linkedTask ? `<button class="link" data-action="task-detail" data-id="${linkedTask.id}">${linkedTask.executionCode}</button>` : "未关联任务"}</div></div>${r.proxyOperator ? `<div class="detail-item full"><label>代办原因</label><div>${r.proxyReason}</div></div>` : ""}</div>${linkedTask ? '<div class="role-note">该记录已用于任务完成认定；关键人、任职、维系时间、关联任务和业务维系人均已锁定。</div>' : '<div class="role-note">该记录未关联任务；允许创建人在权限范围内修改维系方式、维系时间和内容，编辑不会再次自动匹配任务。</div>'}<div class="section-title">沟通摘要</div><p style="font-size:12px;line-height:1.8">${r.summary}</p><div class="section-title">客户反馈</div><p style="font-size:12px;line-height:1.8">${r.feedback}</p><div class="section-title">下一步计划</div><p style="font-size:12px;line-height:1.8">${r.next || "暂无"}</p><div class="section-title">附件</div>${r.attachments.map((x) => `<div class="list-row"><div class="avatar">附</div><div class="list-main"><div class="list-title">${x}</div><div class="list-sub">点击预览或下载将记录操作日志</div></div><button class="btn" data-action="preview-file">预览</button><button class="btn" data-action="download-file">下载</button></div>`).join("") || '<div class="role-note">无附件</div>'}</div><div class="drawer-foot"><button class="btn" data-close>关闭</button>${canEditMaintenanceRecord(r) ? `<button class="btn btn-primary" data-action="edit-record" data-id="${r.id}">编辑记录</button>` : ""}</div>`,
        );
      }
      ...
      function openPersonRecords(personId) {
        ...
        openDrawer(
          `<div class="drawer-head"><div class="modal-title">${person.name} · 全部客户动态</div><button class="icon-btn close" data-close>×</button></div><div class="drawer-body"><div class="detail-hero"><div class="avatar">${person.name[0]}</div><div><div class="detail-name">${person.name}</div><div class="detail-sub">${person.company} · 共 ${records.length} 条维系记录</div></div></div><div class="section-title">维系记录</div><div class="recent-records">${records.map((record) => `<button class="recent-record-item" type="button" data-action="record-detail" data-id="${record.id}"><span class="recent-record-date">${record.date}</span><span class="recent-record-summary">${record.summary}</span><span class="tag blue">${record.method}</span></button>`).join("") || '<div class="role-note">暂无维系记录</div>'}</div></div><div class="drawer-foot"><button class="btn" data-close>关闭</button>${canCreateMaintenanceForPerson(person) ? `<button class="btn btn-primary" data-action="new-record" data-id="${person.id}">新增记录</button>` : ""}</div>`,
        );
      }
