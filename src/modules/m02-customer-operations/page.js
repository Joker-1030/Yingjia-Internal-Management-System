      ...
      function operationVisibleContacts() {
        const activeContacts = scopedContacts();
        if (!currentUser?.fullAccess) return activeContacts;
        const activeIds = new Set(activeContacts.map((person) => person.id));
        return [
          ...activeContacts,
          ...contacts.filter(
            (person) =>
      ...
      function renderOperations() {
        ...
        const people = operationVisibleContacts()
          .filter((person) =>
        ...
        personItemsHost
          ...
          .forEach((item, index) => {
            ...
            if (title && person?.archived)
              title.insertAdjacentHTML(
                "beforeend",
                ' <span class="tag red">已停用</span>',
              );
            else if (title && person && !personHasOverdueTask(person))
              title.insertAdjacentHTML(
                "beforeend",
        ...
        if (selectedPerson) {
          ...
          if (hero && editButton && !selectedPerson.archived)
