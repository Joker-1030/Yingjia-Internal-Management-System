      ...
      function commitPermissionVersion(template, permissions, operations, fields, attachments, reason, type = "保存") {
        if (template.name === "系统管理员") {
          toast("系统管理员模板固定全权限，不允许修改或回滚");
          return false;
        }
      ...
      function renderPermissions() {
        ...
        return (
          pageHead(
            ...
            `<span class="tag green">admin 专属</span>`,
          ) +
          `<section class="panel permission-workspace"><aside class="permission-column"><div class="panel-title">角色模板</div><div class="panel-sub">固定 6 个内置角色</div><input class="input" id="permissionRoleSearch" placeholder="搜索角色" style="margin-top:12px"><div class="permission-role-list">${systemRoleTemplates.map((item) => `<button class="permission-role-item ${item.name === template.name ? "active" : ""}" data-permission-role="${item.name}" data-role-search="${item.name}${roleTemplateScopeText(item)}"><span class="avatar">${item.name[0]}</span><span><strong>${item.name}</strong><span class="list-sub">${roleTemplateScopeText(item)}</span></span><span>›</span></button>`).join("")}</div><div class="role-note" style="margin-top:14px">系统管理员角色固定绑定唯一内置 admin 账号，不进入员工可选系统角色候选。</div></aside><div class="permission-column"><div class="panel-head" style="padding:0"><div><div class="panel-title">${template.name}权限树</div><div class="panel-sub">当前版本 ${currentVersion.id} · ${dirty ? `待保存 ${Object.values(permissionDiff(template, draft)).flat().length} 项变更` : "无未保存变更"}</div></div><span class="spacer"></span><span class="tag ${immutable ? "blue" : "green"}">${immutable ? "固定全权限" : "可编辑"}</span></div><input class="input" id="permissionTreeSearch" placeholder="搜索菜单、操作、字段或附件权限" style="margin:12px 0">${immutable ? '<div class="role-note">系统管理员模板始终全选，不允许取消、停用、删除、转移或回滚。</div>' : ""}<div class="section-title">菜单 / 页面权限</div>${groups}<div class="section-title">操作权限</div>${operations || '<div class="role-note">先勾选页面后配置对应操作。</div>'}<div class="section-title">字段权限</div><div class="permission-tree-group">${fields}</div><div class="section-title">附件权限</div><div class="permission-tree-group">${attachments}</div>${immutable ? '<div class="role-note" style="margin-top:14px">权限授权本身为 admin 专属，不出现在其他角色的可编辑权限树中。</div>' : `<div class="form-group" style="margin-top:16px"><label class="form-label">变更原因 * <span class="panel-sub">5-500 字</span></label><textarea class="input" id="permissionReason" minlength="5" maxlength="500" placeholder="说明本次调整原因">${permissionChangeReason}</textarea></div><div class="permission-actions"><button class="btn" id="discardPermissionChanges" ${dirty ? "" : "disabled"}>撤销修改</button><button class="btn btn-primary" id="saveRoleTemplate" ${dirty ? "" : "disabled"}>预览影响并保存</button></div>`}</div><aside class="permission-column"><div class="panel-head" style="padding:0"><div class="panel-title">权限详情与影响</div></div><div class="permission-summary" style="margin-top:14px"><div class="permission-summary-item"><label>角色编码</label><strong>${permissionRoleCodes[template.name]}</strong></div><div class="permission-summary-item"><label>数据范围类型</label><strong>${roleTemplateScopeText(template)}</strong></div><div class="permission-summary-item"><label>范围计算来源</label><strong>${template.scopeSource}</strong></div><div class="permission-summary-item"><label>数据对象</label><strong>${template.objects.join("、")}</strong></div><div class="permission-summary-item"><label>关联在职员工</label><strong>${employeeCount} 人</strong></div><div class="permission-summary-item"><label>员工单独授权</label><strong>不支持</strong></div></div><div class="section-title">版本历史</div><div class="permission-history">${versions.map((version, index) => `<div class="permission-history-item"><div style="display:flex;align-items:center;gap:8px"><strong>${version.id}</strong>${index === 0 ? '<span class="tag green">当前</span>' : immutable ? '<span class="tag blue">审计快照</span>' : `<button class="link" data-permission-rollback="${version.id}">回滚</button>`}</div><div class="list-sub">${version.type} · ${version.operator} · ${version.time}</div><div class="list-sub">${version.reason}</div><div class="list-sub">页面 ${version.permissions.length} · 操作 ${version.operations?.length || 0} · 字段 ${version.fields.length} · 附件 ${version.attachments?.length || 0}</div></div>`).join("")}</div></aside></section>`
        );
      }
